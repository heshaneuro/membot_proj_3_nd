...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        if (newNode == _nodes.end())
                        {
                            ...
                            //newNode = std::make_unique(new GraphNode(id));
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (*parentNode)->AddEdgeToChildNode(std::unique_ptr<GraphEdge>(edge));
                        }
    ...
    // create instance of chatbot
    //_chatBot = new ChatBot("../images/chatbot.png");
    std::string file_name = "../images/chatbot.png";
    ChatBot _chat_Bot(file_name); //ChatBot("../images/chatbot.png");


    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    _chat_Bot.SetChatLogicHandle(this);

    
    SetChatbotHandle(&_chat_Bot);
    // add chatbot to graph root node
    _chat_Bot.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(_chat_Bot));
